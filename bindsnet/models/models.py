...
from ..network.topology import Connection, LocalConnection, DelayConnection
...
class IncreasingInhibitionDelayNetwork(Network):
    # language=rst
    """"""
    ...
    def __init__(
        ...
        w = 0.3 * torch.rand(self.n_input, self.n_neurons)
        input_output_conn = DelayConnection(
            source=self.layers["X"],
            target=self.layers["Y"],
            w=w,
